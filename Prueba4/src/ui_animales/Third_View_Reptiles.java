...
public class Third_View_Reptiles {
	//
	//ATRIBUTOS
	...
	public Third_View_Reptiles() {
		...
		incluir.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(tfEdad.getText().isEmpty() || tfNombrecientifico.getText().isEmpty() || tfNombrecomun.getText().isEmpty()) {
					...
					Reptiles np = new Reptiles(e, name, namec);
					Lista_Reptiles.getInstances().getLista().add(np);

					
				       try {
				            PreparedStatement pst = cn.prepareStatement("INSERT INTO reptiles(nombrecientifico,nombrecomun,edad) VALUES (?,?,?)");
		...
		modificar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				for (int i = 0; i < Lista_Reptiles.getInstances().getLista().size(); i++) {
					if (c.equalsIgnoreCase(Lista_Reptiles.getInstances().getLista().get(i).getNombrecomun())) {

						Lista_Reptiles.getInstances().getLista().get(i).setEdad(tfEdad.getText());
						Lista_Reptiles.getInstances().getLista().get(i).setNombrecientifico(tfNombrecientifico.getText());
						Lista_Reptiles.getInstances().getLista().get(i).setNombrecomun(tfNombrecomun.getText());
		    	...
		    	try {
		    		PreparedStatement pst = cn.prepareStatement("UPDATE reptiles SET nombrecientifico='"+tfNombrecientifico.getText()+"',edad='"+tfEdad.getText()+"' WHERE nombrecomun='"+tfBuscar.getText()+"'");
		    		pst.executeUpdate();
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				for (int i = 0; i < Lista_Reptiles.getInstances().getLista().size(); i++) {
					if (c.equalsIgnoreCase(Lista_Reptiles.getInstances().getLista().get(i).getNombrecomun())) {
						...
						Lista_Reptiles.getInstances().getLista().remove(i);
			     ...
			     try {
			            ...
			            PreparedStatement pst = cn.prepareStatement("delete from reptiles where nombrecomun = ?");
				...
				if(tfBuscar.getText().isEmpty()) {
					...
					for (int i = 0; i < Lista_Reptiles.getInstances().getLista().size(); i++) {
						if (c.equalsIgnoreCase(Lista_Reptiles.getInstances().getLista().get(i).getNombrecomun())) {
							...
							tfEdad.setText(Lista_Reptiles.getInstances().getLista().get(i).getEdad());
							tfNombrecientifico.setText(Lista_Reptiles.getInstances().getLista().get(i).getNombrecientifico());
							tfNombrecomun.setText(Lista_Reptiles.getInstances().getLista().get(i).getNombrecomun());
				      ...
				      try{
				            ...
				            PreparedStatement pst = cn.prepareStatement("select * from reptiles where nombrecomun = ?");
				            pst.setString(1, tfBuscar.getText().trim());
